...
| ルーティング | `/` をルートとして Start/Game ビューをクライアント内で切り替える単一ルート構成。Setting のみ `/setting` を維持する。 |
| オーディオ状態共有 | Start/Game/Setting で利用する BGM・SE 音量を `AudioPreferenceContext` で一元管理し、root ルート直下で Provider をマウントする。音確認ダイアログや BGM スライダーはこのコンテキスト経由で双方向に同期する。 |
...
| 画面 / ビュー ID | パス例 | 役割 | 備考 |
| --- | --- | --- | --- |
| root.startView | `/` | Start メニューを描画するビュー。ボタン押下で同ページ内の Game ビューへ切り替える。 | ルーティング遷移は発生せず、状態管理でビュースイッチする。 |
| root.gameView | `/` | Bingo 抽選ビュー。Start ビューと同一 DOM 階層に共存し、メニュー操作で切り替える。 | ブラウザのオーディオ制限回避のため Start → Game の操作を同ページ内で完結させる。 |
| setting | `/setting` | 景品情報などの設定を行う独立画面。 | HTML の `<table>` タグ禁止。Grid / Flex で「テーブル風」レイアウトを実現する。 |

### 3.2 React Router v7 でのルーティング要件

| 項目 | 内容 |
| --- | --- |
| ルート定義 | React Router v7 の推奨スタイルに従い、`root.tsx`（Start/Game 複合ビュー）と `/setting` の 2 ルートを定義する。 |
| 画面コンポーネント | StartView・GameView はルートコンポーネント直下で状態切り替えを行う。ビュー固有の JSX / ロジックは `components` 配下へ分離する。 |
| ルートファイルの責務 | データ取得（loader）、フォーム処理（action）、複合ビューのマウントのみ担当。それ以外は `components` / `common` 配下へ分離する。 |

---

## 4. 各画面の詳細仕様

### 4.1 Start ビュー（root 内）

| 項目 | 内容 |
| --- | --- |
| 概要 | アプリ起動時に表示されるメニュー画面。新規ゲーム開始・前回の続き・設定画面遷移を行う。右上に BGM のオン／オフ切り替えアイコンを表示する。 |
| ボタン：はじめから | 押下時に現在のゲーム状態を初期化し、同一ページ内で Game ビューへ切り替える。初期化内容には「これまでの当選番号」「景品の選出状態」などを含める。 |
| ボタン：続きから | ブラウザローカル（例：localStorage）に保存された最新のゲーム状態を読み込み、同一ページ内で Game ビューへ切り替える。保存データがない場合の挙動（新規開始など）は別途定義。 |
| ボタン：設定 | `/setting` に遷移する。 |
| BGM オン／オフ | 右上に BGM トグルアイコンを表示する。`@mui/icons-material` の音量系アイコンを利用し、オン／オフ状態を視覚的に区別する。 |
| BGM 設定の保存 | BGM のオン／オフ状態はブラウザローカルに保存し、アプリ再訪問時にも状態を復元する。`AudioPreferenceContext` を経由して Game ビューの BGM 音量とも常に同期する。 |
| 音確認ダイアログ | 初回表示時に「音なし/音あり/×」を提示し、選択結果を `AudioPreferenceContext` と音声詳細設定へ一括反映する。音無しを選んだ場合はドラムロール・シンバル・読み上げ音量も 0 にする。 |
...
| 音量設定 | Start ビューと同じ `AudioPreferenceContext` を参照する。Game 画面の BGM/SE スライダーや音量ダイアログで更新した値は即座に Start ビューの表示・音確認ダイアログへ反映する。 |
...
| ディレクトリ | 役割 |
| --- | --- |
| `app/routes` | React Router v7 のルートモジュールを配置。各 `route.tsx` ファイルには `loader`, `action`, `default export component` のみ記載する。 |
| `app/components` | 画面共通で利用する UI コンポーネント（ボタン、モーダル、レイアウトコンポーネントなど）を配置する。共通コンポーネントは必ずここに置く。 |
| `app/common` | 共通ロジック・ユーティリティ（カスタムフック、ドメインロジック、バリデーション、定数など）を配置する。共通コンポーネント以外の共通部品は必ずここに置く。 |
...
| ルーティング詳細 | 実際に採用するルーティングとファイル構成の初期案 | `/`（Start/Game 切り替え）と `/setting` の 2 ルート構成。`app/routes/start-game.tsx` で `AudioPreferenceProvider` をマウントし、Game ビュー切替と BGM 制御を担う。 |
