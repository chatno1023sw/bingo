...
export const fetchWithRetry = async (url: string, maxRetries: number = 3): Promise<any> => {
  const attempt = async (retriesLeft: number): Promise<any> => {
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, 1000 * (maxRetries - retriesLeft + 1)));
