...
async function apiCall<T>(url: string, options?: RequestInit, errorContext?: string): Promise<ApiResult<T>> {
  try {
...
export const fetchUserData = (userId: string) => apiCall(`/api/users/${userId}`, undefined, "fetching user data");

export const fetchProductData = (productId: string) =>
  apiCall(`/api/products/${productId}`, undefined, "fetching product data");

export const fetchOrderData = (orderId: string) => apiCall(`/api/orders/${orderId}`, undefined, "fetching order data");
