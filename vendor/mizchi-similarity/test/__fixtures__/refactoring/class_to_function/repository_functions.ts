...
export function findById<T extends Entity>(state: RepositoryState<T>, id: string): T | null {
...
export function deleteItem<T extends Entity>(state: RepositoryState<T>, id: string): boolean {
...
export function findAll<T extends Entity>(state: RepositoryState<T>): T[] {
  return Array.from(state.items.values());
}

export function findByPredicate<T extends Entity>(state: RepositoryState<T>, predicate: (item: T) => boolean): T[] {
  return findAll(state).filter(predicate);
}

export function count<T extends Entity>(state: RepositoryState<T>): number {
  return state.items.size;
}

export function clear<T extends Entity>(state: RepositoryState<T>): void {
  state.items.clear();
